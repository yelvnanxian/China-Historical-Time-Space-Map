...
console.log(JSON.stringify({models:input.models.map(p=>({id:p.id,name:getBoundaryDisplayLabel(p).name,key:key(getBoundaryDisplayLabel(p).name)})),points:input.points.map(p=>({id:p.id,key:key(p.name)}))}));"""
    ...
    for f in current:
        ...
        candidate_ids = sorted(set(named_ids + ([linked_county] if linked_county else [])))
        excluded_ids = []
        ...
        for area_id in candidate_ids + excluded_ids:
            ...
            if area_id in candidate_ids:
                candidates.append({'boundaryId': area_id, 'name': model_names[area_id]['name'], 'sourceName': area['sourceName'],
                                   ...
                                   'basis': 'reviewed-source-hierarchy' if reviewed else 'source-name' if area_id in named_ids else 'documented-county-link'})
        ...
        reason = {
            ...
            'outside': '原始治所点在所有同名县模型面之外；尚无可靠县面对应，不自动选最近县面。',
    ...
    for area in models:
        ...
        for point in incoming[ident]:
            ...
            (excluded if (other and not inside) or reviewed and reviewed['boundaryId'] != ident else relevant).append(record)
        ...
        if len(inside) == 1:
            ...
            status = 'outside'; reason = '唯一尚未对应其他同名县面的来源点在本模型之外，且741与755年都保留同一原始点。两套来源存在点面冲突，此面仅作诊断参照。'
        elif relevant:
            ...
            if excluded:
                reason += '同名点已对应其他县面，已排除，不能据此把异地同名县判为冲突。'
    ...
    result = {'version': '1.0.0', 'sourceYear': 755, 'boundaryYear': 741, 'bySettlement': by_settlement, 'byBoundary': by_boundary,
              'statistics': statistics, 'sources': [{'id': ident, 'title': title, 'url': url, 'path': path, 'sha256': sha(path)} for ident, title, url, path in source_specs],
              'notes': ['Hartwell以1990年县域为构件近似表示古代行政区，官方说明其来源和方法与CHGIS时序点完全不同，许多边界可能不适合作为真实辖区。',
                        ...
                        '繁简及行政通名正规化使用应用现有规则；异名仅使用已有countyBoundaryId证据。两处吉阳按已核对的旧唐书州郡隶属排除异地同名。距离不参与配对。',
